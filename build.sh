...
(
  ...
  javac -cp lib/javax.mail.jar -sourcepath src -d build src/ru/david/room/client/Client.java -encoding UTF-8
) && (
  echo "Сборка клиентского jar...";
  jar cfm jars/ClientApp.jar metadata/CLIENT_MANIFEST.MF -C build .
) && (
  echo "Компиляция сервера...";
  javac -cp lib/javax.mail.jar -sourcepath src -d build src/ru/david/room/server/Server.java -encoding UTF-8
) && (

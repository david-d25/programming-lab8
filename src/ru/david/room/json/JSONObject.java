...
public class JSONObject extends JSONEntity {
    ...
    /**
     * Возвращает элемент json-объекта по указанному ключу. Если элемент не найден, бросает {@link NoSuchElementException}
     * @param key ключ
     * @param onNull сообщение потенциально бросаемого исключения
     * @return элемент, с которым ассоциируется ключ
     * @throws NoSuchElementException Если нет объекта, ассоциированного с данным ключом
     */
    public JSONEntity getItemNotNull(String key, String onNull) throws NoSuchElementException {
        JSONEntity item = items.get(key);
        if (item != null)
            return item;
        throw new NoSuchElementException(onNull);
    }
